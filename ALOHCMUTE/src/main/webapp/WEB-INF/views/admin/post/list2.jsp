					...
					<c:if test="${post.access_modifier != 2}">
						<div class="card gedf-card">
								...
								<div class="d-flex justify-content-between align-items-center">
									<div class="d-flex justify-content-between align-items-center">
										...
										<%-- <div class="h5 m-0">${user.value[0]}</div> --%>
											<div>
												 <div class="h7 text-muted small">${post.postDate}</div>
							...
							<c:forEach var="cmtcount" items="${cmtcount}">
								<c:if test="${post == cmtcount.key}">
									<a href="/posts/post/comment/${post.postid}/${currentUser}/${userid}" class="card-link"><i class="fa fa-comment"></i> Comment(${cmtcount.value})</a>
								</c:if>
							</c:forEach>
